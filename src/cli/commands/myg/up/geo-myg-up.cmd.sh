...
export up_command_file_path="${BASH_SOURCE[0]}"
# The full path to the directory that this file is in.
export up_command_directory_path="$(dirname "${BASH_SOURCE[0]}")"
...
#        to automatically using lock a lock file.
# [Log Functions]
...
@geo_up_doc() {
  ...
  doc_cmd 'up'
    ...
    doc_cmd_sub_cmd 'subcommand1'
          doc_cmd_sub_cmd_desc 'An example subcommand with its own function called _geo_up_subcommand1'
    doc_cmd_sub_cmd 'subcommand1'
          doc_cmd_sub_cmd_desc 'Another example subcommand with its own function called _geo_up_subcommand2'
    doc_cmd_sub_cmd 'edit'
          doc_cmd_sub_cmd_desc "Opens up the command file for this command in VS Code."

      doc_cmd_examples_title
          doc_cmd_example 'geo up test -a "some_argument"'
          doc_cmd_example 'geo up test -b "option argument" "subcommand argument"'
          doc_cmd_example 'geo up subcommand1'
          doc_cmd_example 'geo up edit'
}
@geo_up() {
    ...
    # For example 'getopts "b:"' will require the -b option to take an arg: 'geo up -b 22 ...'.
    ...
    # This case statement runs the command's subcommand based on what was passed in by the user (e.g. geo up test).
    case "$cmd" in
        test )
            log::success "Command 'up' is now available in geo-cli. Add your logic to the command file at $up_command_file_path"
            prompt_continue "Edit your command now in VS Code? (Y|n): " && code $up_command_file_path
            ;;
        subcommand1 | subcmd1 )
            # Passes all remaining arguments to the _geo_up__subcommand1 subcommand function to keep things easy to read.
...
_geo_up__subcommand1() {
    ...
    log::debug "_geo_up__subcommand1 $*"
}
