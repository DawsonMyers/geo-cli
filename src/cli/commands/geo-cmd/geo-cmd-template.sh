...
export {{new_command_name}}_command_file_path="${BASH_SOURCE[0]}"
# The full path to the directory that this file is in.
export {{new_command_name}}_command_directory_path="$(dirname "${BASH_SOURCE[0]}")"
...
@geo_{{new_command_name}}_doc() {
  ...
  doc_cmd '{{new_command_name}}'
    ...
    doc_cmd_sub_cmd 'subcommand1'
          doc_cmd_sub_cmd_desc 'An example subcommand with its own function called _geo_{{new_command_name}}_subcommand1'
    doc_cmd_sub_cmd 'subcommand1'
          doc_cmd_sub_cmd_desc 'Another example subcommand with its own function called _geo_{{new_command_name}}_subcommand2'
    doc_cmd_sub_cmd 'edit'
      ...
      doc_cmd_examples_title
          ...
          doc_cmd_example 'geo {{new_command_name}} test -b "option argument" "subcommand argument"'
...
@geo_{{new_command_name}}() {
    ...
    if [[ -z $cmd ]]; then
         ...
         prompt_continue "Edit command now in VS Code? (Y|n): " \
            && code "${{new_command_name}}_file_path"
         return 1
    fi
    ...
    case "$cmd" in
        test )
            log::success "Command '{{new_command_name}}' is now available in geo-cli. Add your logic to the command file at ${{new_command_name}}_command_file_path"
            prompt_continue "Edit your command now in VS Code? (Y|n): " && code ${{new_command_name}}_command_file_path
            ;;
        subcommand1 | subcmd1 )
            # Passes all remaining arguments to the _geo_{{new_command_name}}__subcommand1 subcommand function to keep things easy to read.
            ...
            _geo_{{new_command_name}}__subcommand1 "$@"
            ;;
        subcommand2 | subcmd2 )
            _geo_{{new_command_name}}_subcommand2 "$@"
            ;;
        edit )
            _geo_cmd__edit {{new_command_name}}
            ;;
        * )
...
#   2: arg description
# Returns:
#   A colour-formatted debug log
_geo_{{new_command_name}}__subcommand1() {
    # Access positional arguments like this: $1, $2, etc.
    log::debug "_geo_{{new_command_name}}__subcommand1 $*"
}
...
#   A colour-formatted debug log
_geo_{{new_command_name}}_subcommand2() {
    log::debug "_geo_{{new_command_name}}_subcommand2 $*"
}
