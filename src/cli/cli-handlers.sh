...
_geo_ar__copy_pgAdmin_server_config() {
    ...
    (
        ...
        for file in *$file_pattern*; do
            ...
            # echo "$text" > "# This file was automatically generated by geo-cli. It is recreated each time a PG port (5433, by default, but can be set via the 'geo ar tunnel -L -P <port>' option) is bound to a remote server over ssh+IAP"
            echo "$text" > "$path"
            chmod 0600 "$path"
        ...
        if [[ -f $passfile && $passfile =~ ^iap ]]; then
            ...
            [[ -f $user_pgpass_file && ! -f $user_pgpass_file.bac ]] \
                && cp $user_pgpass_file $user_pgpass_file.bac
            cp "$passfile" "$HOME/.pgpass"
        fi
    )
    # set +x
